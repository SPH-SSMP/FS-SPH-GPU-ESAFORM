...
void geometry_rotate(std::vector<mesh_triangle> &triangles, std::vector<vec3_t> &positions, rotation_axis axis, float_x angle);
...
void geometry_get_bb(const float4_t * positions, unsigned int num_part, vec3_t &bbmin, vec3_t &bbmax, float_x safety = 0.);

void geometry_rotate(std::vector<vec3_t> &positions, rotation_axis axis, float_x angle);
