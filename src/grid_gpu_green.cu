...
__global__ static void reorder_data(
	...
	if (idx < num_part) {
		...
		float_x  h          = particles.h[sorted_index];
		float_x  rho        = particles.rho[sorted_index];
		mat3x3_t S          = particles.S[sorted_index];
		float_x  eps_pl     = particles.eps_pl[sorted_index];
		float_x  eps_pl_dot = particles.eps_pl_dot[sorted_index];
		float_x  T          = particles.T[sorted_index];
		float_x  fixed      = particles.fixed[sorted_index];
		float_x  blanked    = particles.blanked[sorted_index];
		float_x  tool_particle = particles.tool_particle[sorted_index];

		float3_t pos_t  = particles.pos_t[sorted_index];
		float3_t vel_t  = particles.vel_t[sorted_index];
		float_x  rho_t  = particles.rho_t[sorted_index];
		mat3x3_t S_t    = particles.S_t[sorted_index];
		float_x  T_t    = particles.T_t[sorted_index];
...
void grid_gpu_green::alloc_buffer(int num_cell, int num_part) {
	...
	cudaMalloc((void**) &m_buffer->h, sizeof(float_x)*num_part);
	cudaMalloc((void**) &m_buffer->rho, sizeof(float_x)*num_part);
	cudaMalloc((void**) &m_buffer->S, sizeof(mat3x3_t)*num_part);
	cudaMalloc((void**) &m_buffer->eps_pl, sizeof(float_x)*num_part);
	cudaMalloc((void**) &m_buffer->eps_pl_dot, sizeof(float_x)*num_part);
	cudaMalloc((void**) &m_buffer->T, sizeof(float_x)*num_part);
	cudaMalloc((void**) &m_buffer->fixed, sizeof(float_x)*num_part);
	cudaMalloc((void**) &m_buffer->blanked, sizeof(float_x)*num_part);
	cudaMalloc((void**) &m_buffer->tool_particle, sizeof(float_x)*num_part);

	cudaMalloc((void**) &m_buffer->pos_t, sizeof(float4_t)*num_part);
	cudaMalloc((void**) &m_buffer->vel_t, sizeof(float4_t)*num_part);
	cudaMalloc((void**) &m_buffer->rho_t, sizeof(float_x)*num_part);
	cudaMalloc((void**) &m_buffer->S_t, sizeof(mat3x3_t)*num_part);
	cudaMalloc((void**) &m_buffer->T_t, sizeof(float_x)*num_part);
...
grid_gpu_green::grid_gpu_green(int num_part, float3_t bbmin, float3_t bbmax, float_x h) :
