...
__device__ __inline__
float3_t Multiply(float_x s, float3_t const& U) {
...
__device__ __inline__
float3_t Divide(float3_t const& U, float_x s) {
    float_x invS = (float_x)1 / s;
    float3_t division = make_float3_t( U.x * invS, U.y * invS, U.z * invS );
    return division;
}

__device__ __inline__
float_x Dot(float3_t const& U, float3_t const& V) {
    float_x dot = U.x * V.x + U.y * V.y + U.z * V.z;
    return dot;
}
__device__ __inline__
void ComputeOrthogonalComplement(float3_t const& W, float3_t& U, float3_t& V) {
    ...
    if (fabs(W.x) > fabs(W.y)) {
        ...
        invLength = (float_x)1 / sqrt(W.x * W.x + W.z * W.z);
        U = make_float3_t( -W.z * invLength, (float_x)0, +W.x * invLength );
    } else {
        // The component of maximum absolute value is either W.y or W.z.
        invLength = (float_x)1 / sqrt(W.y * W.y + W.z * W.z);
        U = make_float3_t( (float_x)0, +W.z * invLength, -W.y * invLength );
    }
    V = Cross(W, U);
}

__device__
void ComputeEigenvector0(float_x a00, float_x a01,
    float_x a02, float_x a11, float_x a12, float_x a22, float_x eval0, float3_t& evec0)  {
    ...
    float_x d0 = Dot(r0xr1, r0xr1);
    float_x d1 = Dot(r0xr2, r0xr2);
    float_x d2 = Dot(r1xr2, r1xr2);
...
__device__
void ComputeEigenvector1(float_x a00, float_x a01,
    float_x a02, float_x a11, float_x a12, float_x a22, float3_t const& evec0,
    float_x eval1, float3_t& evec1) {
    ...
    float_x m00 = U.x * AU.x + U.y * AU.y + U.z * AU.z - eval1;
    float_x m01 = U.x * AV.x + U.y * AV.y + U.z * AV.z;
    float_x m11 = V.x * AV.x + V.y * AV.y + V.z * AV.z - eval1;
    ...
    float_x absM00 = fabs(m00);
    float_x absM01 = fabs(m01);
    float_x absM11 = fabs(m11);
    float_x maxAbsComp;
    if (absM00 >= absM11)
    {
        maxAbsComp = fmax(absM00, absM01);
        if (maxAbsComp > (float_x)0)
        {
            if (absM00 >= absM01)
            {
                m01 /= m00;
                m00 = (float_x)1 / sqrt((float_x)1 + m01 * m01);
                m01 *= m00;
            }
            else
            {
                m00 /= m01;
                m01 = (float_x)1 / sqrt((float_x)1 + m00 * m00);
                m00 *= m01;
            }
    ...
    else
    {
        ...
        if (maxAbsComp > (float_x)0)
        {
            ...
            {
                m01 /= m11;
                m11 = (float_x)1 / sqrt((float_x)1 + m01 * m01);
                m01 *= m11;
            }
            else
            {
                m11 /= m01;
                m01 = (float_x)1 / sqrt((float_x)1 + m11 * m11);
                m11 *= m01;
            }
...
__device__
void solve_eigen(float_x a00, float_x a01, float_x a02, float_x a11, float_x a12, float_x a22,
		float3_t &eval, float3_t &e1, float3_t &e2, float3_t &e3) {

	float_x max0 = fmax(fabs(a00), fabs(a01));
	float_x max1 = fmax(fabs(a02), fabs(a11));
	float_x max2 = fmax(fabs(a12), fabs(a22));
	float_x maxAbsElement = fmax(fmax(max0, max1), max2);
	...
	float_x invMaxAbsElement = (float_x) 1 / maxAbsElement;
	...
	float_x norm = a01 * a01 + a02 * a02 + a12 * a12;
	if (norm > (float_x)0)
	{
		...
		float_x traceDiv3 = (a00 + a11 + a22) / (float_x)3;
		float_x b00 = a00 - traceDiv3;
		float_x b11 = a11 - traceDiv3;
		float_x b22 = a22 - traceDiv3;
		float_x denom = sqrt((b00 * b00 + b11 * b11 + b22 * b22 + norm * (float_x)2) / (float_x)6);
		float_x c00 = b11 * b22 - a12 * a12;
		float_x c01 = a01 * b22 - a12 * a02;
		float_x c02 = a01 * a12 - b11 * a02;
		float_x det = (b00 * c00 - a01 * c01 + a02 * c02) / (denom * denom * denom);
		float_x halfDet = det * (float_x)0.5;
		halfDet = fmin(fmax(halfDet, (float_x)-1), (float_x)1);
		...
		float_x angle = acos(halfDet) / (float_x)3;
		float_x const twoThirdsPi = (float_x)2.09439510239319549;
		float_x beta2 = cos(angle) * (float_x)2;
		float_x beta0 = cos(angle + twoThirdsPi) * (float_x)2;
		float_x beta1 = -(beta0 + beta2);
	...
	else
	{
		...
		eval.y = a11;
		eval.z = a22;
		e1 = make_float3_t( (float_x)1, (float_x)0, (float_x)0 );
		e2 = make_float3_t( (float_x)0, (float_x)1, (float_x)0 );
		e3 = make_float3_t( (float_x)0, (float_x)0, (float_x)1 );
	}
