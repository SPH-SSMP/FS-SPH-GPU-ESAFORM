...
static float_x dist_squared(vec3_t p1, vec3_t p2) {
...
bool tool_3d::contact(vec3_t qp, vec3_t &cp, vec3_t &dir) const {
	...
	if (m_cells[box_idx] != m_cells[box_idx+1]) {
		...
		for (unsigned int pidx = m_cells[box_idx]; pidx < m_cells[box_idx+1]; pidx++) {
			...
			float_x cur_dist = dist_squared(cp_tri, qp);
	...
	for (; iz < m_nz; ++iz) {
		...
		for (unsigned int pidx = m_cells[box_idx]; pidx < m_cells[box_idx+1]; pidx++) {
			...
			if (inter) {	//triangle may be in more than one box. make sure that intersection is counted only once
				...
				float_x lo_x = m_grid.bbmin_x() + ix*m_grid.dx();
				float_x lo_y = m_grid.bbmin_y() + iy*m_grid.dx();
				float_x lo_z = m_grid.bbmin_z() + iz*m_grid.dx();

				float_x hi_x = m_grid.bbmin_x() + (ix+1)*m_grid.dx();
				float_x hi_y = m_grid.bbmin_y() + (iy+1)*m_grid.dx();
				float_x hi_z = m_grid.bbmin_z() + (iz+1)*m_grid.dx();
...
const std::vector<vec3_t> tool_3d::sample(float_x dx) const {

	vec3_t bbmin, bbmax;
	get_bbox(bbmin, bbmax);

	float_x lx = bbmax.x - bbmin.x;
	float_x ly = bbmax.y - bbmin.y;
	float_x lz = bbmax.z - bbmin.z;
	...
	for (unsigned int i = 0; i < nx; i++) {
		for (unsigned int j = 0; j < ny; j++) {
			for (unsigned int k = 0; k < nz; k++) {
				float_x px = bbmin.x + i*dx;
				float_x py = bbmin.y + j*dx;
				float_x pz = bbmin.z + k*dx;
...
tool_3d::tool_3d(std::vector<mesh_triangle> triangles, std::vector<vec3_t> positions, float_x h_avg) {
