...
float_x mesh_triangle::max_h() const {
	float_x l1 = glm::length(p1 - p2);
	float_x l2 = glm::length(p2 - p3);
	float_x l3 = glm::length(p3 - p1);

	return fmax(fmax(l1,l2),l3);
}

vec3_t mesh_triangle::center() const {
	return float_x(1./3.)*(vec3_t(p1) + vec3_t(p2) + vec3_t(p3));
}
...
void mesh_triangle::closest_vertex(vec3_t p, vec3_t &cp, unsigned int &idx) const {
	float_x d1 = glm::length(p - vec3_t(p1));
	float_x d2 = glm::length(p - vec3_t(p2));
	float_x d3 = glm::length(p - vec3_t(p3));
...
bool mesh_triangle::cover_aabb(vec3_t bbmin, vec3_t bbmax) const {
	vec3_t boxcenter   = float_x(0.5)*(bbmin+bbmax);
	vec3_t boxhalfsize = float_x(0.5)*(bbmax-bbmin);
	...
	float_x min,max,p0,p1,p2,rad;

	float_x fex = fabs(e0.x);
	float_x fey = fabs(e0.y);
	float_x fez = fabs(e0.z);
...
bool mesh_triangle::ray_intersect(vec3_t o, vec3_t dir, float_x &t) const {
...
static vec3_t angle_average_vertex_normal(const std::vector<unsigned int> &triangle_idx, const std::vector<mesh_triangle> &triangles, vec3_t pos) {
	...
	for (auto it = triangle_idx.begin(); it != triangle_idx.end(); it++) {
		...
		float_x angle = acos(fabs(glm::dot(f1p, f2p)/(glm::length(f1p)*glm::length(f2p))));
...
static void print_triangle_normals(FILE* fp, mesh_triangle t) {
	...
	vec3_t e1 = float_x(0.5)*(t.p1 + t.p2);
	vec3_t e2 = float_x(0.5)*(t.p2 + t.p3);
	vec3_t e3 = float_x(0.5)*(t.p3 + t.p1);

	fprintf(fp, "%f %f %f %f %f %f\n", e1.x, e1.y, e1.z, t.ne1.x, t.ne1.y, t.ne1.z);
	fprintf(fp, "%f %f %f %f %f %f\n", e2.x, e2.y, e2.z, t.ne2.x, t.ne2.y, t.ne2.z);
	fprintf(fp, "%f %f %f %f %f %f\n", e3.x, e3.y, e3.z, t.ne3.x, t.ne3.y, t.ne3.z);

	vec3_t f = float_x(1./3.)*(t.p1 + t.p2 + t.p3);
...
float_x mesh_average_edge_length(const std::vector<mesh_triangle> &triangles) {
	float_x lmin = DBL_MAX;
	float_x lmax = 0;

	for (auto it = triangles.begin(); it != triangles.end(); ++it) {
		float_x l1 = glm::length(it->p1-it->p2);
		float_x l2 = glm::length(it->p2-it->p3);
		float_x l3 = glm::length(it->p3-it->p1);
