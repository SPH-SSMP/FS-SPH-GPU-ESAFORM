...
__global__ void init_cooling(particle_gpu particles, inistate_struct inistate)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N_init)
		return;
...
__global__ void predict(particle_gpu particles, inistate_struct inistate, float_x dt)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N_init)
		return;
...
__global__ void predict_cooling(particle_gpu particles, inistate_struct inistate, float_x dt)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N_init)
		return;
...
__global__ void correct(particle_gpu particles, inistate_struct inistate, float_x dt)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N_init)
		return;
...
__global__ void correct_cooling(particle_gpu particles, inistate_struct inistate, float_x dt)
{
	unsigned int pidx = blockIdx.x * blockDim.x + threadIdx.x;
	if (pidx >= particles.N_init)
		return;
...
__global__ void do_reset_normals(particle_gpu particles)
{
	const unsigned int N = particles.N;
...
void leap_frog::step(particle_gpu *particles, grid_base *g, bool record_forces)
{
	...
	do_reset_normals<<<dG, dB>>>(*particles);
	...
	/* 	for (auto globaltool : global_tool)
		{ // Iterate over contacting tool(s)
			globaltool->update_tool();
			globaltool->compute_contact_force(particles, record_forces);
		} */
	// contact_force(particles);

	interactions_rod_force_Songwon(particles, cell_start,
								   cell_end, g->num_cell(), forces, fr_heat_gen);

	// material_fric_heat_gen(particles, global_tool[0]->get_vel());

	perform_boundary_conditions(particles);
	perform_merge_conditions_thermal(particles);
...
void leap_frog::step_cooling(particle_gpu *particles, grid_base *g)
{
	const unsigned int block_size = BLOCK_SIZE;
	dim3 dG((particles->N_init + block_size - 1) / block_size);
	dim3 dB(block_size);
	...
	init_cooling<<<dG, dB>>>(*particles, inistate);
	predict_cooling<<<dG, dB>>>(*particles, inistate, global_time_dt);

#ifdef Thermal_Conduction_PSE
	interactions_heat_pse(particles, cell_start, cell_end, g->num_cell());
#else
	interactions_heat_Brookshaw(particles, cell_start, cell_end, g->num_cell());
#endif

	correct_cooling<<<dG, dB>>>(*particles, inistate, global_time_dt);
	perform_boundary_conditions_thermal(particles);
...
leap_frog::leap_frog(unsigned int num_part, unsigned int num_cell)
{
	...
	cudaMalloc((void **)&rho_init, sizeof(float_x) * num_part);
	cudaMalloc((void **)&T_init, sizeof(float_x) * num_part);

	cudaMalloc((void **)&cell_start, sizeof(int) * num_cell);
	cudaMalloc((void **)&cell_end, sizeof(int) * num_cell);

	cudaMalloc((void **)&forces, sizeof(float3_t));
	cudaMalloc((void **)&fr_heat_gen, sizeof(float));
}
