...
protected:
	//geometry
	float_x m_dx = 0.;
	float_x m_lx = 0., m_ly = 0., m_lz = 0.;
	long int m_nx = 0, m_ny = 0, m_nz = 0;
	long int m_num_cell = 0;

	float_x m_bbmin_x = 0., m_bbmax_x = 0.;
	float_x m_bbmin_y = 0., m_bbmax_y = 0.;
	float_x m_bbmin_z = 0., m_bbmax_z = 0.;

	float_x m_bbmin_x_init = 0., m_bbmax_x_init = 0.;
	float_x m_bbmin_y_init = 0., m_bbmax_y_init = 0.;
	float_x m_bbmin_z_init = 0., m_bbmax_z_init = 0.;
	...
	void set_geometry(float3_t bbmin, float3_t bbmax, float_x h);
...
public:
	...
	float_x bbmin_x() const;
	float_x bbmin_y() const;
	float_x bbmin_z() const;
	float_x bbmax_x() const;
	float_x bbmax_y() const;
	float_x bbmax_z() const;
	float_x dx() const;
	bool is_locked() const;
	long int num_cell() const;

	void assign_hashes(particle_gpu *partilces) const;
	void update_geometry(particle_gpu *particles, float_x kernel_width = 2.0);
	...
	grid_base(int num_part, float3_t bbmin, float3_t bbmax, float_x h);
	virtual ~grid_base();
};
