...
particle_gpu *setup_ring_contact(int nbox, grid_base **grid) {
	...
	float_x ri = 0.035;
	float_x ro = 0.04;
	float_x spacing = ro + 0.001;

	float_x dx = 2*ro/(nbox-1);
	float_x hdx = 1.7;
	...
	for (int i = 0; i < nbox; i++) {
		for (int j = 0; j < nbox; j++) {
			for (int k = 0; k < nbox; k++) {
				float_x px = -ro+i*dx; float_x py = -ro+j*dx; float_x pz = -ro+k*dx;
				float_x dist = sqrt(px*px + py*py + pz*pz);
				if (dist < ro && dist >= ri) {
					float4_t posl, posr;
	...
	float_x vel_ring = 250.;
	float_x c0 = sqrt(phys.K/phys.rho0);

	float_x delta_t_max = 0.8*hdx*dx/(vel_ring + c0);
	...
	*grid = new grid_gpu_green(n, make_float3_t(-.2,-.06,-.06), make_float3_t(+.1,+.06, +.06), hdx*dx);
...

	{
		...
		for (auto &it : triangles) {
			...
			vec3_t center = float_x(1./3.)*(it.p1 + it.p2 + it.p3);
	...
	for (int i = 0; i < nbox; i++) {
		for (int j = 0; j < nbox; j++) {
			for (int k = 0; k < nheight; k++) {
				float_x px = -ro+i*dx; float_x py = -ro+j*dx; float_x pz = k*dx;
				float_x dist = sqrt(px*px + py*py);
				if (dist < ro && dist >= ri) {
					float4_t posl, posr;
...
particle_gpu *setup_plastic_ball_plastic_wall_impact(int nbox, grid_base **grid) {
	...
	float_x l = 0.1;
	float_x wall_thickness = 1/fac*l;
	float_x r = 0.2*l;

	float_x dx = wall_thickness/(nx_wall-1);
	float_x dy = l/(ny_wall-1);
	float_x dz = l/(nz_wall-1);

	int nx_ball = ceil(2*r/dx);

	float_x hdx = 1.7;
	float_x ddx = fmax(dx,fmax(dy,dz));
	...
	for (int i = 0; i < nx_wall; i++) {
		for (int j = 0; j < ny_wall; j++) {
			for (int k = 0; k < nz_wall; k++) {
				float_x px = i*dx; float_x py = j*dy; float_x pz = k*dz;
				float4_t cur_pos;
	...
	for (int i = 0; i < nx_ball; i++) {
		for (int j = 0; j < nx_ball; j++) {
			for (int k = 0; k < nx_ball; k++) {
				float_x px = i*dx-r; float_x py = j*dx-r; float_x pz = k*dx-r;
				float_x dist = sqrt(px*px + py*py + pz*pz);
				if (dist <= r) {
					float4_t cur_pos;
	...
	*grid = new grid_gpu_green(n, make_float3_t(bbmin.x, bbmin.y, bbmin.z), make_float3_t(bbmax.x, bbmax.y, bbmax.z), hdx*ddx);
	...
	float_x c0 = sqrt(phys.K/phys.rho0);
	float_x delta_t_max = 0.3*hdx*dz/(v_impact + c0);
