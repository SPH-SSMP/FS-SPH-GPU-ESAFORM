...
particle_gpu *setup_single_grain_5tool(grid_base **grid) {
	...
	float_x target_feed = 20*1e-6*100;
	int depth_fac = 3;
	float_x depth_wp = depth_fac*target_feed;

	float_x num_particles_in_feed = 12;     //12 = 1.5M is resolution used in paper
	...
	float_x ly_tool = bbmax_tool.y - bbmin_tool.y;
	geometry_translate(triangles, positions, vec3_t(0.,-bbmin_tool.y - ly_tool/2., 0.));		//center tool to y zero

	float_x nudge = 6.4e-4;
	float_x lx_tool = bbmax_tool.x - bbmin_tool.x;
	...
	float_x edge_diamond = 80*1e-6*100;
	float_x lx = 3*edge_diamond;
	float_x ly = 3*edge_diamond + 2*edge_diamond + 1*edge_diamond;
	float_x lz = depth_wp;

	float_x sp_length = (separate) ? 0.5*edge_diamond : 0;
...
//	geometry_translate(triangles_t_r, positions_t_r, vec3_t(-lx, 0., 0.));
	...
	for (int i = 0; i < nx; i++) {
		for (int j = 0; j < ny; j++) {
			for (int k = 0; k < nz; k++) {
				float_x px = i*dz; float_x py = j*dz-ly/2.; float_x pz = - k*dz;
				float4_t cur_pos;
...
//	n = pos_it;
	...
	float_x rho0_tool = 3.5;	// https://en.wikipedia.org/wiki/Diamond
...
//	float_x v_tool = 8e-5;	//speed akbari
	float_x v_tool = 3e-3;	//speed KW
//	float_x v_tool = 570*100/1e6; //reckless speed

	float_x friction_coeff = 0.35;	//fric ruttiumann
//	float_x friction_coeff = 0.6;	//fric akbari
//	float_x friction_coeff = 0.8;	//fric akbari increased
	...
	float_x avg_h = mesh_average_edge_length(triangles);
	...
	if (interp_steady_temp) {
		...
		while (std::getline(infile, line)) {
			...
			float_x x = std::stod (tokens[0], NULL);
			float_x y = std::stod (tokens[1], NULL);
			float_x z = std::stod (tokens[2], NULL);
			float_x t = std::stod (tokens[3], NULL);
		...
		float_x ddx = fabs(steady_state_tool[0].x - steady_state_tool[1].x);
		float_x ddy = fabs(steady_state_tool[0].y - steady_state_tool[1].y);
		float_x ddz = fabs(steady_state_tool[0].z - steady_state_tool[1].z);
	...
	if (sample_tools) {
		...
		for (auto it : cpu_tools) {
			...
			if (interp_steady_temp) {
				...
				interp_temps(shifted_steady_tool, dx_steady*dx_steady*dx_steady, std::max(float_x(1.5)*dx_steady, hdx*dz), cur_samples_flt_4);
			}
		...
		rho    = (float_x*)  realloc(rho,    sizeof(float_x)*(n+n_tool));
		T      = (float_x*)  realloc(T,      sizeof(float_x)*(n+n_tool));
		h      = (float_x*)  realloc(h,      sizeof(float_x)*(n+n_tool));
		fixed  = (float_x*)  realloc(fixed,  sizeof(float_x)*(n+n_tool));
		tool_p = (float_x*)  realloc(tool_p, sizeof(float_x)*(n+n_tool));
		...
		for (int i = n; i < n+n_tool; i++) {
			...
			float_x d_l_l = fabs(glm::dot(nrm_l_l, qp-p1_l_l)/glm::length(nrm_l_l));
			float_x d_l_m = fabs(glm::dot(nrm_l_m, qp-p1_l_m)/glm::length(nrm_l_m));
			float_x d_l_r = fabs(glm::dot(nrm_l_r, qp-p1_l_r)/glm::length(nrm_l_r));
			float_x d_t_l = fabs(glm::dot(nrm_t_l, qp-p1_t_l)/glm::length(nrm_t_l));
			float_x d_t_r = fabs(glm::dot(nrm_t_r, qp-p1_t_r)/glm::length(nrm_t_r));
	...
	const float_x CFL = 0.3;
	float_x delta_t_max = CFL*hdx*dz/(sqrt(v_tool) + c0);
...
particle_gpu *setup_single_grain_1tool_realscale(grid_base **grid) {
	...
	float_x target_feed = 30*1e-6*100;	//30 mu in cm
	int depth_fac = 3;
	float_x depth_wp = depth_fac*target_feed;

	float_x num_particles_in_feed = 5;		//5 = 1.2M particles, which was used for the paper
	int nz = depth_fac*num_particles_in_feed;
	...
	float_x lx = 2*270*1e-6*100;	//from measured base surface
	float_x ly = 2*320*1e-6*100;
	float_x lz = depth_wp;
...
particle_gpu *setup_single_grain_1tool(grid_base **grid) {
	...
	float_x num_particles_in_feed = 18;     //18 = ~2M reference simulation
	int nz = depth_fac*num_particles_in_feed;
	...
	float_x lx = 2*80*1e-6*100;	//full diamond measures
	float_x ly = 2*80*1e-6*100;
	float_x lz = depth_wp;
...
//	float_x v_tool = 8e-5;	//speed akbari
	float_x v_tool = 3e-3;	//speed KW
//	float_x v_tool = 570*100/1e6; //breakneck speed
...
particle_gpu *setup_single_grain_1tool_trml_steady(grid_base **grid) {
	...
	float_x target_feed = 20*1e-6*100;	//20 mu in cm
	int depth_fac = 3;
	float_x depth_wp = depth_fac*target_feed;

	float_x num_particles_in_feed = 6;
	int nz = depth_fac*num_particles_in_feed;
...
//	geometry_scale(triangles, positions, vec3_t(1./10000.0,1./10000.0,1./10000.0));	//micron to cm
	...
	float_x lx = 80*80*1e-6*100;	//full diamond measures
	float_x ly = 2*80*1e-6*100;
	float_x lz = depth_wp;
	...
	float_x  *h     = new float_x[n+n_tool];
	float_x  *rho   = new float_x[n+n_tool];
	float_x  *fixed = new float_x[n+n_tool];
	float_x  *T     = new float_x[n+n_tool];
	float_x  *tool_p = new float_x[n+n_tool];
	...
	unsigned int Nr_BC_particles = 0;	// Counter boundary particles
	{
		...
		for (unsigned int i = 0; i < n+n_tool; i++) {	// define boundary particles
			...
			if (tool_p[i] == 1.) {	//thermal sinks at tool bounds
				...
				vec3_t qp(pos[i].x, pos[i].y, pos[i].z);
				float_x d = glm::dot(nrm, qp-p1)/glm::length(nrm);
