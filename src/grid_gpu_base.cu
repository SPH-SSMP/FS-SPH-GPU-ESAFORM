...
__global__ static void compute_hashes(const float4_t * pos, float_x *blanked, float_x *in_tool, int *hashes, int count,
								float_x dx, long int nx, long int ny, long int nz,
								float_x bbmin_x, float_x bbmin_y, float_x bbmin_z, long int max_cell, bool hard_blank) {
	...
	if (idx < count) {
		...
		float_x _px = pos[idx].x;
		float_x _py = pos[idx].y;
		float_x _pz = pos[idx].z;
...
float_x grid_base::bbmin_x() const {
	return m_bbmin_x;
}

float_x grid_base::bbmin_y() const {
	return m_bbmin_y;
}

float_x grid_base::bbmin_z() const {
	return m_bbmin_z;
}

float_x grid_base::bbmax_x() const {
	return m_bbmax_x;
}

float_x grid_base::bbmax_y() const {
	return m_bbmax_y;
}

float_x grid_base::bbmax_z() const {
	return m_bbmax_z;
}

float_x grid_base::dx() const {
	return m_dx;
}
...
void grid_base::update_geometry(particle_gpu *particles, float_x kernel_width) {
	...
	thrust::device_ptr<float_x> maxh = thrust::max_element(t_h, t_h+N);
...
void grid_base::set_geometry(float3_t bbmin, float3_t bbmax, float_x h) {
...
grid_base::grid_base(int num_part, float3_t bbmin, float3_t bbmax, float_x h) {
	m_num_part = num_part;
