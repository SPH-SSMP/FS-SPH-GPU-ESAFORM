...
private:
	...
	float_x m_max_vel_squared = FLT_MAX;
...
public:
	...
	blanking(float_x max_vel);

	blanking(vec3_t bbmin, vec3_t bbmax, float_x max_vel);
	blanking(vec3_t bbmin, vec3_t bbmax, vec3_t bbvel, float_x max_vel);
	...
	float_x get_max_vel_squared() const;
};
