...
extern float_x global_time_current;
...
public:
	...
	void set_mu(float_x mu);

	// set contact stiffness
	void set_contact_alpha(float_x alpha);
	...
	void sample(const tool_3d* tool, std::vector<float4_t> &samples, float_x dx);
	...
	void set_birth_death(float_x birth, float_x death);

	tool_3d_gpu(const tool_3d* tool, vec3_t vel,
			phys_constants phys, float_x alpha = 0.1);

	tool_3d_gpu(const tool_3d* tool, float_x alpha = 0.1);
