...
__constant__ float_x  friction_mu;
__constant__ float_x  contact_alpha;
__constant__ float_x  slave_mass;
__constant__ float3_t tool_vel;
__constant__ float_x usui_K;
__constant__ float_x usui_xi;
...
__global__ void do_update_tool(mesh_triangle_gpu triangles, int N, float_x cur_t, vec3_t vel) {
...
static __device__ float_x unhash_pos(vec3_t pos, int &ix, int &iy, int &iz) {
...
static __device__ vec3_t closest_on_segment(vec3_t p1, vec3_t p2, vec3_t p) {
	float_x dx = p1.x - p2.x;
	float_x dy = p1.y - p2.y;
	float_x dz = p1.z - p2.z;

	float_x ns2 = dx*dx + dy*dy + dz*dz;

	if (ns2 == 0.) {
		return p1;
	}

	float_x t = ((p2.x - p1.x)*(p.x - p1.x) +
			(p2.y - p1.y)*(p.y - p1.y) +
...
static __device__ float_x distsquared(vec3_t p1, vec3_t p2) {
	float_x dx = p1.x - p2.x;
	float_x dy = p1.y - p2.y;
	float_x dz = p1.z - p2.z;
...
static __device__ void comp_closest_point(mesh_triangle_gpu triangles, int idx, vec3_t pos, vec3_t &cp, vec3_t &np) {
	...
	float_x B = glm::dot(p1p3, p1p2);
	float_x E = glm::dot(p1p2, p1p2);
	float_x C = glm::dot(p1p3, p1p3);
	...
	float_x A = glm::dot(p1p3, pp1);
	float_x D = glm::dot(p1p2, pp1);

	float_x t1 = (D*C - A*B)/det;
	float_x t2 = (A*E - D*B)/det;
...
static __device__ bool triangle_ray_intersect(mesh_triangle_gpu triangles, int idx, vec3_t o, vec3_t dir) {
    ...
    const float_x EPSILON = 0.0000001;
    ...
    float_x t = f * glm::dot(edge2, q);
    return (t > EPSILON);
}

static __device__ bool triangle_ray_intersect(mesh_triangle_gpu triangles, int idx, vec3_t o, vec3_t dir, float_x &t) {
	//Möller Trombore intersection algorithm from
	//https://en.wikipedia.org/wiki/M%C3%B6ller%E2%80%93Trumbore_intersection_algorithm
...
__device__ bool establish_contact_by_normal(mesh_triangle_gpu triangles, const int  *__restrict__ cells, vec3_t qp, vec3_t &cp, vec3_t &n, int num_tri, int num_cell) {
	...
	if (cells[box_idx] != cells[box_idx+1]) {
		...
		for (int pidx = cells[box_idx]; pidx < cells[box_idx+1]; pidx++) {
			...
			float_x cur_dist = distsquared(cp_tri, qp);
	...
	float_x sign = (glm::dot(diff,normal) > 0.) ? 1. : -1.;
	...
	for (; iz < m_nz; ++iz) {
		...
		for (unsigned int pidx = cells[box_idx]; pidx < cells[box_idx+1]; pidx++) {
			...
			if (inter) { //triangle may be in more than one box. make sure that intersection is counted only once
				...
				float_x lo_x = geometry.bbmin_x + ix*geometry.dx;
				float_x lo_y = geometry.bbmin_y + iy*geometry.dx;
				float_x lo_z = geometry.bbmin_z + iz*geometry.dx;

				float_x hi_x = geometry.bbmin_x + (ix+1)*geometry.dx;
				float_x hi_y = geometry.bbmin_y + (iy+1)*geometry.dx;
				float_x hi_z = geometry.bbmin_z + (iz+1)*geometry.dx;
...
__global__ void do_compute_contact_forces_n2(int tool_idx, particle_gpu particles, mesh_triangle_gpu triangles,
	...
	float_x min_dist = FLT_MAX;

	for (int i = 0; i < num_tri; i++) {
		vec3_t cp;
		vec3_t np;
		comp_closest_point(triangles, i, qp, cp, np);
		float_x dist = distsquared(cp, qp);
		if (dist < min_dist) {
			min_dist = dist;
			closest_point = cp;
			normal = np;
	...
	float_x dt2 = dt*dt;
	float_x gN  = glm::dot((qp-closest_point), normal);
	...
	if (record_forces) {
		float_x fx = float(fN.x+fT.x);
		float_x fy = float(fN.y+fT.y);
		float_x fz = float(fN.z+fT.z);
...
__global__ void do_compute_contact_forces(int tool_idx, particle_gpu particles, mesh_triangle_gpu triangles,
		const int *__restrict__ cells, float_x dt, int num_tri, int num_cell,
...
void tool_3d_gpu::set_mu(float_x mu) {
	assert(mu > 0.);
	m_mu = mu;
	cudaMemcpyToSymbol(friction_mu, &m_mu, sizeof(float_x), 0, cudaMemcpyHostToDevice);
}
void tool_3d_gpu::set_contact_alpha(float_x alpha) {
	assert(alpha > 0.);
	cudaMemcpyToSymbol(contact_alpha, &alpha, sizeof(float_x), 0);
}

void tool_3d_gpu::set_birth_death(float_x birth, float_x death) {
	m_birth = birth;
	m_death = death;
}
...
void tool_3d_gpu::set_physics(phys_constants phys) {
	...
	cudaMemcpyToSymbol(slave_mass,    &phys.mass, sizeof(float_x), 0);
}

tool_3d_gpu::tool_3d_gpu(const tool_3d* tool, vec3_t vel, phys_constants phys, float_x alpha) {
...
tool_3d_gpu::tool_3d_gpu(const tool_3d* tool, float_x alpha) {
...
void tool_3d_gpu::sample(const tool_3d* tool, std::vector<float4_t> &samples, float_x dx) {
